...
namespace Ecommerce.API.Controllers
{
    ...
    public class ProductsController : BaseApiController
    {
        ...
        {
            // Validate that ProductBrand exists
            var brand = await _unitOfWork.Repository<ProductBrand>()
                .GetByIdAsync(creationDto.ProductBrandId);
            if (brand is null)
                return BadRequest(new ApiResponse((int)HttpStatusCode.BadRequest, "Invalid Product Brand Id"));

            // Validate that ProductType exists
            var type = await _unitOfWork.Repository<ProductType>()
                .GetByIdAsync(creationDto.ProductTypeId);
            if (type is null)
                return BadRequest(new ApiResponse((int)HttpStatusCode.BadRequest, "Invalid Product Type Id"));
        ...
        public async Task<ActionResult<ProductResponseDto>> Update([FromForm] ProductUpdateDto updateDto)
        {
            ...
            var hasNewImage = updateDto.Image is not null;
            var oldImage = product.PictureUrl;

            _mapper.Map(updateDto, product);

            if (hasNewImage)
                product.PictureUrl = await _fileService.SaveFileAsync(updateDto.Image!, "products");
